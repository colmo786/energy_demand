...
def get_hourly_demand(**kwargs):
    ...
    process_ok = _get_hourly_demand(date=kwargs['logical_date'], database=database, host=host
                    , user=user, password=password, port=port, verbose=True)
    print('INFO get_hourly_demand execution: ', kwargs['logical_date'])
    if not process_ok:
        raise ValueError()
    return
...
def calculate_hourly_demand_forecast(**kwargs):
    ...
    print('INFO calculate_hourly_demand_forecast execution init: ', kwargs['logical_date'])
    process_ok, error_txt = _calculate_hourly_demand_forecast(timestamp_max=kwargs['logical_date'], database=database, host=host, user=user, password=password
...
dag = DAG(
    ...
    schedule_interval='55 * * * *' # Every hour at minute 55
)
